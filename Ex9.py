...
for word in pass_options:
    response = requests.post("https://playground.learnqa.ru/ajax/api/get_secret_password_homework", data={'login': 'super_admin', 'password': word})
    ...
    if response2.text == 'You are authorized':
        print(f"Correct password is: {json.loads(response.text)['password']}")
        break
